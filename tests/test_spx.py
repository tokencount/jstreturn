...
class SpxContractTests(unittest.TestCase):
    ...
    def test_ui_contains_buttons_sections_and_http_error_handling(self):
        ...
        self.assertIn("'/api/inventory/image/' + encodeURIComponent(item.sku)", html)
        self.assertIn('@click="openImagePreview({ imageUrl: \'/api/inventory/image/\' + encodeURIComponent(item.sku), code: item.sku })"', html)
